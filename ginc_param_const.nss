...
// wired 10/28/2010 make PC_LEADER multi-player-friendly 
...
const string TARGET_PC_AREA		= "$PC_AREA";		// PCSpeaker's Area
...
object GetTarget(string sTarget, string sDefault=TARGET_OWNER)
{
	...
	if (IsParameterConstant(sTarget))
	{
		...
		else if (TARGET_PC_LEADER == sIdentifier)	oTarget = GetFactionLeader(GetPCSpeaker());
		...
		else if ( TARGET_PC_AREA == sIdentifier) oTarget = GetArea(GetPCSpeaker());
		else
		{
