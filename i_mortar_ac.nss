...
const string FAIRY_DUST				= "no_tag";
const string SHADOW_REAVER_BONES	= "no_tag";
...
void Druagoth(object oTarget, object oPC)
{
if (!GetSkillRank(SKILL_CRAFT_ALCHEMY,oPC,FALSE) >= 18)
{
SendMessageToPC(oPC,"The creature's blood radiates with divine essence. Perhaps, with sufficient knowledge, an alchemist could produce something out of this.");
return;
}
else
{
SendMessageToPC(oPC,"You successfully create a concoction out of the Druagoth's blood. The divine essence is sure to heal any wound!");
DestroyObject(oTarget);
object oNew = CreateItemOnObject("trollishconcoction",oPC,1,"",FALSE);
SetFirstName(oNew,"Druagoth Blood");
SendMessageToPC(oPC,"Acquired Item: Druagoth Blood");
}
}
...
*/	
	...
	else if (iObjType == OBJECT_TYPE_ITEM)
	{
		...
		else if (sTargetTag == "carci_druagblood")
			Druagoth(oTarget,oPC);
		else
