...
void main()
{
	...
	float fDur     = IntToFloat(60); // wired 2014/05/03 60 seconds = 10 rounds // 18 seconds/3 rounds
	...
	*/
	...
	if (!bHasFeat) // If no Handle Poison feat, then 5% chance of failure
	{
		...
		// wired 2014/05/03 ... not sure why do this if invis/stealthed or not fighting
		if ( InvisibleTrue(oPC) == FALSE  || GetIsInCombat(oPC) == TRUE )
		{	
	...
	// Check to see if the weapon already has poison applied to it.
   	if (!GetItemHasItemProperty(oTarget, ITEM_PROPERTY_ON_MONSTER_HIT))
   	{
		...
		DelayCommand(fDur,FloatingTextStringOnCreature("Weapon is no longer poisoned",oPC,FALSE));   
    }
    else
    {
